import {DiagnosticsPage,prepare} from './ecosystem.js';
...
const routes=[['image','Create image'],['cinema','Cinema studio'],['models','Models & engines'],['workflows','Video & workflows'],['gallery','Gallery & queue'],['branding','White label'],['status','Engine status']];
...
async function navigate(page){
 ...
 if(page==='models')view=ModelsPage(s);else if(page==='branding')view=BrandingPage(s);else if(page==='workflows')view=WorkflowsPage(s);else if(page==='gallery')view=await GalleryPage(s);else if(page==='cinema')view=CinemaStudio();else if(page==='status')view=DiagnosticsPage();else view=ImageStudio();if(current!==version)return;content.append(view);
...
const readiness=document.createElement('div');readiness.className='origin-readiness';readiness.innerHTML='<span role="status">Checking your active model…</span><button id="ready-setup">Set up active model</button><button id="ready-details">Engine status</button>';document.querySelector('.origin-nav').after(readiness);
readiness.querySelector('#ready-setup').onclick=async()=>{try{await prepare(window.originActiveModel)}catch(e){window.originNotice(e.message,true)}};
readiness.querySelector('#ready-details').onclick=()=>navigate('status');
let polling=false,lastReady=0;
async function poll(){if(polling||window.parent.privacyLocked)return;polling=true;
try{
 const [s,n]=await Promise.all([localAPI('/api/studio/state',{}),localAPI('/api/images/state',{})]);setModels(s);
 const prep=s.preparation||{};const i=['downloading','extracting','starting'].includes(s.install.status)?s.install:n.busy&&!n.ready?n.setup:null;const bar=document.querySelector('#origin-work');bar.hidden=!i&&prep.status!=='running';
 if(i||prep.status==='running'){bar.querySelector('span').textContent=(i?.message||prep.message)+(i?.total?' · '+bytes(i.done||0)+' / '+bytes(i.total):'');const p=bar.querySelector('progress');if(!i?.total||i.status==='extracting'||i.status==='starting')p.removeAttribute('value');else{p.max=i.total;p.value=i.done}}
 if(route==='models'){for(const row of s.catalog){const card=document.querySelector('[data-model="'+CSS.escape(row.model.id)+'"]');if(card){card.querySelector('.origin-card-head span:last-child').textContent=row.installed?'Files available':'Download required';card.querySelector('[data-install]').textContent=row.installed?'Verify files':'Download model pack'}}}
 if(prep.status==='running'){readiness.querySelector('span').textContent=prep.stage+' · '+prep.message;readiness.querySelector('#ready-setup').disabled=true}
 else {readiness.querySelector('#ready-setup').disabled=false;if(Date.now()-lastReady>6000){lastReady=Date.now();const r=await localAPI('/api/studio/readiness',{});readiness.querySelector('span').textContent=r.ready?'Ready · '+(r.device||r.engine):r.message;readiness.querySelector('#ready-setup').hidden=r.ready;readiness.classList.toggle('is-ready',r.ready)}}
 if(s.install.status==='failed')window.originNotice(s.install.message,true);
 if(prep.status==='failed'&&window.originSetupTarget){window.originSetupTarget=null;window.originNotice(prep.message,true)}
 if(prep.status==='ready'&&window.originSetupTarget===prep.model){window.originSetupTarget=null;lastReady=0;window.originNotice(prep.message);await navigate('image')}
}catch(e){if(!String(e.message).includes('locked'))window.originNotice(e.message,true)}finally{polling=false}}
setInterval(poll,2200);poll();
navigate('image');
