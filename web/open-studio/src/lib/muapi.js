...
export class MuapiClient{
 ...
 async generateImage(p){
  ...
  const [a,b]=(p.aspect_ratio||'1:1').split(':').map(Number);let width=p.width||m.width||512,height=p.height||Math.round(width*b/a/16)*16;
  if(!p.width&&!p.height&&height>width){height=width;width=Math.round(height*a/b/16)*16}
  ...
  for(let i=0;i<count;i++){const j=await localAPI('/api/studio/generate',{...request,seed:request.seed===-1?-1:request.seed+i});if(!first)first=j;if(p.onRequestId)p.onRequestId(j.id)}
