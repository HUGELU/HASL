...
export function ImageStudio() {
    ...
    // --- State ---
    const draft=window.originDraft;window.originDraft=null;
    const defaultModel = t2iModels.find(m=>m.id===draft?.model)||t2iModels[0];
    ...
    hero.innerHTML = `
        ...
        <p class="text-secondary text-sm font-medium tracking-wide opacity-60">Generate locally. Refine with a reference. Finish at export resolution.</p>
    `;
    ...
    const textarea = document.createElement('textarea');textarea.setAttribute('aria-label','Image prompt');textarea.id='origin-image-prompt';if(draft?.prompt)textarea.value=draft.prompt;
    textarea.placeholder = 'Describe the image you want to create';
            ...
            <div class="flex gap-4 flex-wrap">
                <div class="flex-1 min-w-[200px] flex flex-col gap-2">
                    <div class="flex items-center justify-between">
                        ...
                        <span id="guidance-value" class="text-xs font-bold text-primary">${guidanceScale}</span>
                    </div>
                    <input type="range" id="guidance-slider" min="1" max="20" step="0.5" value="${guidanceScale}" 
                ...
                <div class="flex-1 min-w-[200px] flex flex-col gap-2">
                    <div class="flex items-center justify-between">
                        ...
                        <span id="steps-value" class="text-xs font-bold text-primary">${steps}</span>
                    </div>
                    <input type="range" id="steps-slider" min="1" max="50" step="1" value="${steps}" 
                        class="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-primary">
                </div>
            </div>
            ...
            const renderModels = (filter = '') => {
                ...
                filtered.forEach(m => {
                    ...
                    item.onclick = (e) => {
                        ...
                        guidanceScale=m.guidance;steps=m.steps;
                        stepsSlider.value=steps;stepsValue.textContent=steps;guidanceSlider.value=guidanceScale;guidanceValue.textContent=guidanceScale;modelControls();
    ...
    const addToHistory = (entry) => {
        ...
        // Durable history is saved by the ORIGIN server.
    ...
    const renderHistory = () => {
        ...
        generationHistory.forEach((entry, idx) => {
            ...
            thumb.innerHTML = `
                <img src="${entry.url}" alt="${escapeHTML(entry.prompt?.substring(0, 30) || 'Generated')}" class="w-full aspect-square object-cover">
    ...
    (async () => {
        ...
        const apiKey = true; // ORIGIN enforces the local session and PIN at the server.
    ...
    generateBtn.onclick = async () => {
        ...
        try {
            ...
            if (imageMode) {
                const genParams = {
                    model: selectedModel,
                    negative_prompt: negativePrompt, guidance_scale: guidanceScale, steps, seed,
                    width: customWidth, height: customHeight, strength: referenceStrength/100,
                    lora: selectedLora, lora_weight: loraWeight, batch_count: batchCount,
                    images_list: uploadedImageUrls,
                    image_url: uploadedImageUrls[0], // backward compat for single-image models
                    aspect_ratio: selectedAr,
                    onRequestId: (rid) => {
                        capturedRequestId = rid;
                        savePendingJob({ requestId: rid, studioType: 'image', historyMeta, maxAttempts: 7200, interval: 2000, submittedAt: Date.now() });
                    }
                };
