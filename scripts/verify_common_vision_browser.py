...
def main():
 ...
 try:
  with App(a.binary) as app,sync_playwright() as pw:
   ...
   try:
    page.goto(app.root+'/#'+app.key);page.get_by_role('button',name='Common Vision',exact=True).click();expect(page.locator('#cv-status')).to_contain_text('Local workspace ready')
    ...
    page.reload();page.get_by_role('button',name='Common Vision',exact=True).click();expect(page.locator('#cv-outcomes')).to_contain_text('Synthetic pilot');expect(page.locator('#cv-framework')).to_contain_text('ADOPTED LOCALLY')
